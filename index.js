...
function addCourseToStudent(student, course) {
  // Your code here
  let r = student.courses.length;
  //console.log(r);
  student.courses[r] = "Physics";
  return student;
}
...
function listAllCourses(students) {
  ...
  for (let i = 0; i <= students.length - 1; i++) {
    for (let x = 0; x <= students[i].courses.length - 1; x++) {
      if (uniqueCourses.includes(students[i].courses[x])) {
      } else uniqueCourses.push(students[i].courses[x]);
    }
  }
...
function removeCourseFromStudent(student, course) {
  ...
  let res = student.courses.filter((studentCourse) => {
    if (studentCourse != course) return studentCourse;
  });

  student.courses = res;
  return student;
}
console.log("withour sci", removeCourseFromStudent(students[6], "Science"));
...
console.log("#7. ", findStudentById(10, students));
